...
--
local PrinterMade = 15 -- How much this printer will make every interval (how many seconds you decided above)
--
local PrinterColor = Color(133,119,63) -- Color of this printer
--
local PrinterName = "Copper Printer" -- Name of this printer
--
...
function ENT:SetupDataTables()
	self:NetworkVar("Int",0,"CopperStored")
...
if SERVER then
	...
	function ENT:Think()
		if CurTime() > self.intvl + interval then
			self.intvl = CurTime()
			self:SetCopperStored(self:GetCopperStored()+PrinterMade)
		end
	...
	function ENT:Use(act,ply)
		local money = self:GetCopperStored()
		if IsValid(ply) && ply:IsPlayer() && money > 0 then
			self:SetCopperStored(0)
			ply:addMoney(money)
...
if CLIENT then
	...
	function ENT:Draw()
		...
		if IsValid(Owner) and Owner:Nick() then 
			Owner = Owner:Nick()
			if string.len(Owner) > 9 then
				Owner = string.Left( Owner, 9 ).."..."
			end
		else 
			Owner = "Unknown" 
		end
		...
		local MoneyText = string.Left(GAMEMODE.Config.currency..string.Comma(self:GetCopperStored()),7)
		...
		cam.Start3D2D(pos+ang:Up()*17,ang,0.1)
			...
			draw.SimpleTextOutlined(string.Left(PrinterName,14),"SmallInfo",-145,-85,Color(255,255,255),0,0,2,Color(0,0,0))
		cam.End3D2D()
	end
end