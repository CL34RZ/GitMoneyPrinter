...
--
local PrinterMade = 55 -- How much this printer will make every interval (how many seconds you decided above)
--
local PrinterColor = Color(255,51,0) -- Color of this printer
--
local PrinterName = "Ruby Printer" -- Name of this printer
--
...
function ENT:SetupDataTables()
	self:NetworkVar("Int",0,"RubyStored")
...
if SERVER then
	...
	function ENT:Think()
		if CurTime() > self.intvl + interval then
			self.intvl = CurTime()
			self:SetRubyStored(self:GetRubyStored()+PrinterMade)
		end
	...
	function ENT:Use(act,ply)
		local money = self:GetRubyStored()
		if IsValid(ply) && ply:IsPlayer() && money > 0 then
			self:SetRubyStored(0)
			ply:addMoney(money)
...
if CLIENT then
	...
	function ENT:Draw()
		...
		local MoneyText = string.Left(GAMEMODE.Config.currency..string.Comma(self:GetRubyStored()),7)
