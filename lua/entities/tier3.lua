...
--
local PrinterMade = 35 -- How much this printer will make every interval (how many seconds you decided above)
--
local PrinterColor = Color(255,215,0) -- Color of this printer
--
local PrinterName = "Gold Printer" -- Name of this printer
--
...
function ENT:SetupDataTables()
	self:NetworkVar("Int",0,"GoldStored")
...
if SERVER then
	...
	function ENT:Think()
		if CurTime() > self.intvl + interval then
			self.intvl = CurTime()
			self:SetGoldStored(self:GetGoldStored()+PrinterMade)
		end
	...
	function ENT:Use(act,ply)
		local money = self:GetGoldStored()
		if IsValid(ply) && ply:IsPlayer() && money > 0 then
			self:SetGoldStored(0)
			ply:addMoney(money)
...
if CLIENT then
	...
	function ENT:Draw()
		...
		local MoneyText = string.Left(GAMEMODE.Config.currency..string.Comma(self:GetGoldStored()),7)
